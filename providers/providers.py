...
class Provider:
    ...
    async def loop_fetch_proxies(self):
        ...
        for proxy in self._pipe():
            if proxy not in self.fetched_proxies:
                await self.pool.put(proxy)

    def _pipe(self):
        for url in url2urls(self.url):
            for proxy in self.fetch_on_page(url):
                yield proxy

    def url2urls(self, url):
        yield url

    def fetch_on_page(self, url, data=None, headers=None, method='GET'):
